...
Return 5-6 skills. Make them HIGHLY SPECIFIC to ${target_role}.`;
        ...
        // Save result
        await db_1.supabase.from('career_assessments').insert({
            user_id: req.userId,
...
router.get('/last', auth_1.authMiddleware, async (req, res) => {
    try {
...
router.post('/complete-skill', auth_1.authMiddleware, async (req, res) => {
    try {
        const { skill_name } = req.body;
        if (!skill_name)
            return res.status(400).json({ message: 'skill_name required' });
        ...
        const { data: profile } = await db_1.supabase
            ...
            .eq('user_id', req.userId)
            .single();
        const existing = profile?.completed_skills || [];
        const updated = existing.includes(skill_name)
            ? existing.filter((s) => s !== skill_name) // toggle off
            : [...existing, skill_name]; // toggle on
        await db_1.supabase.from('profiles').update({ completed_skills: updated }).eq('user_id', req.userId);
...
router.get('/completed-skills', auth_1.authMiddleware, async (req, res) => {
    try {
        const { data } = await db_1.supabase
            .from('profiles')
            .select('completed_skills')
...
router.post('/generate-test', auth_1.authMiddleware, async (req, res) => {
    try {
