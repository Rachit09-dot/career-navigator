...
async function fetchJSearchJobs(query, location = 'India') {
...
function transformJSearchJob(job) {
    ...
    if (src.includes('linkedin'))
        ...
        source = 'indeed';
    else if (src.includes('glassdoor'))
        source = 'glassdoor';
...
function fieldToKeyword(field) {
    const f = field.toLowerCase();
    if (f.includes('engineer') || f.includes('cse') || f.includes('it') || f.includes('computer') || f.includes('software'))
        return 'Software Engineer';
    if (f.includes('medical') || f.includes('mbbs') || f.includes('health'))
        return 'Healthcare';
    if (f.includes('commerce') || f.includes('bba') || f.includes('finance') || f.includes('bcom'))
        return 'Business Analyst';
    if (f.includes('design') || f.includes('creative'))
        return 'UI UX Designer';
    if (f.includes('data') || f.includes('analytics'))
        return 'Data Analyst';
    if (f.includes('mass communication') || f.includes('media') || f.includes('journalism'))
        return 'Journalist';
    if (f.includes('law') || f.includes('legal'))
        return 'Legal Associate';
    if (f.includes('education') || f.includes('teaching'))
        return 'Teacher';
    if (f.includes('hotel') || f.includes('hospitality'))
        return 'Hotel Management';
    if (f.includes('agriculture'))
        return 'Agriculture Officer';
    if (f.includes('sports') || f.includes('physical'))
        return 'Sports Coach';
    if (f.includes('social work'))
        return 'Social Worker';
    if (f.includes('defence') || f.includes('military') || f.includes('paramilitary'))
        return 'Defence';
    if (f.includes('civil service') || f.includes('government') || f.includes('upsc') || f.includes('ias'))
        return 'Government Jobs';
    if (f.includes('fine art') || f.includes('performing'))
        return 'Artist';
    if (f.includes('paramedical') || f.includes('allied health'))
        return 'Paramedic';
    if (f.includes('diploma') || f.includes('iti') || f.includes('vocational'))
        return 'Technician';
    if (f.includes('pure science') || f.includes('physics') || f.includes('chemistry') || f.includes('biology'))
        return 'Research Scientist';
    if (f.includes('art') || f.includes('humanities'))
...
function getSourceLogo(url) {
    ...
    if (url.includes('monster'))
        return 'monster';
    if (url.includes('shine'))
        return 'shine';
    if (url.includes('timesjobs'))
        return 'timesjobs';
    return 'other';
}
...
router.get('/', auth_1.authMiddleware, async (req, res) => {
    try {
        const { q, field, location, page = '1' } = req.query;
        ...
        let userSkills = [];
        try {
            const { data: profile } = await db_1.supabase.from('profiles').select('skills').eq('user_id', req.userId).single();
            if (profile?.skills && Array.isArray(profile.skills)) {
                userSkills = [...profile.skills];
            }
            const { data: completedGaps } = await db_1.supabase.from('skill_gaps').select('completed_skills').eq('user_id', req.userId).order('created_at', { ascending: false }).limit(1);
            if (completedGaps && completedGaps.length > 0 && Array.isArray(completedGaps[0].completed_skills)) {
                const gapSkills = completedGaps[0].completed_skills.map((s) => typeof s === 'string' ? s : s.skill || s.name || '');
        ...
        const processJobs = (jobs) => {
            ...
            const processed = jobs.map(job => {
                ...
                if (matchCount === 0)
                    matchPercentage = Math.floor(Math.random() * 20) + 20; // Random 20-40% if no exact keyword match
...
router.get('/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        const { data: job } = await db_1.supabase
            .from('jobs')
            .select('*')
            .eq('id', req.params.id)
            .single();
...
router.post('/:id/apply', auth_1.authMiddleware, async (req, res) => {
    try {
