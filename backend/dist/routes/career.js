...
router.post('/assessment', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        if (lastCall && now - lastCall < 10 * 60 * 1000) {
            ...
            const { data: cached } = await db_1.supabase
...
[
  {
    ...
    "path": "How to get there in 1-2 sentences"
        ...
        await db_1.supabase.from('profiles').update({
...
router.get('/assessment', auth_1.authMiddleware, async (req, res) => {
    try {
...
function getFallbackResults(answers) {
